...
4. run command ./start-local-environment.sh (On Mac or Linux use ./start-local-environment-arm.sh)
5. Wait for command to finish building
6. Docker desktop should have something like this(add in docker hierarchy)
7. cd into test-suite folder
8. run command ./start-local.sh (this will build the test harness)
...
```
- Certifications for the test harness can be found in integration-adaptor-111\test-suite\certificates\certs.zip